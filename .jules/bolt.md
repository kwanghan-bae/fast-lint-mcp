...
## 2025-02-13 - [Vitest Mocks and restoreAllMocks]
**Learning:** When using `vi.restoreAllMocks()` in `afterEach` (or `vi.clearAllMocks()` mixed with `restoreAllMocks()`), it completely destroys the mocked implementation set via `vi.mock()`, replacing it with `undefined` for subsequent tests, leading to `Cannot read properties of undefined` in mocked function calls.
**Action:** Use `vi.clearAllMocks()` instead of `vi.restoreAllMocks()` if the intent is only to clear mock call history, preserving the actual mocked implementation created via `vi.mock()` for other tests in the suite.
