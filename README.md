# Fast-Lint-MCP (QualityGate) 🚀 v2.1.1

**"AI가 만든 기술 부채를 AI 스스로 해결하게 만드는 지능형 품질 검속 서버"**

Fast-Lint-MCP는 AI 에이전트(Claude, Gemini 등)가 작성한 코드의 품질을 실시간으로 감시하고, 정해진 기준을 통과하지 못할 경우 리팩토링을 강제하는 **Model Context Protocol (MCP)** 서버입니다. v2.1.1에서는 **현업 시니어 개발자의 실전 피드백**을 반영하여 스코프 분석 정밀화와 테스트 파일 예외 처리 기능을 도입했습니다.

---

## ✨ 주요 기능 (Key Features)

### 1. ⚡ Zero-Cache 실시간 분석 아키텍처
...
### 2. 🇰🇷 Korean First 가독성 및 주석 규칙 - v2.1.1 UP!
AI 에이전트가 생성하는 코드의 가독성을 보장하기 위해 한국어 주석 작성을 강력히 강제합니다.
*   **맥락 인식 규칙 (Context-aware)**: `.test.js`, `.spec.ts` 등 테스트 파일은 주석 강제 대상에서 제외하여 소음을 최소화했습니다.
*   **스코프 정밀 분석**: 함수 내부의 지역 변수를 전역 변수로 오인하던 문제를 해결하여, 실제 외부에 노출되는 주요 심볼에 대해서만 주석을 요구합니다.
*   **범용 선언 지원**: ESM(`export`) 뿐만 아니라 Node.js의 **CommonJS(`module.exports`, `exports`)** 패턴까지 완벽히 인식합니다.
*   **클래스 및 멤버 전수 검사**: 클래스, 메서드, 멤버 변수(`Field`), 인터페이스, 타입 별칭에 대해 한글 주석을 검사합니다.

### 3. 📊 시각적 품질 리포트 (Pretty Reports)
MCP 클라이언트(Gemini, Claude 등)에서 보기 좋게 렌더링되는 **Markdown 기반의 유려한 리포트**를 제공합니다.
*   **분석 근거 명시**: 리포트 하단에 **서버 버전(v2.1.1)과 분석된 파일의 선택 기준(Git staging 등)**을 명시하여 분석의 투명성을 높였습니다.
...
## 🤖 AI 에이전트 작업 규약 (Agent Protocol v2.1.1)
