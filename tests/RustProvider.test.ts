...
describe('RustProvider', () => {
  ...
  it('TC 3.2: Violation Generation - Should return COMPLEXITY and SIZE violations for complex file', async () => {
    ...
    const readabilityViolationParams = violations.find(
      (v) => v.type === 'READABILITY' && v.message.includes('파라미터')
    );
    expect(readabilityViolationParams).toBeDefined();

    const readabilityViolationLines = violations.find(
      (v) => v.type === 'READABILITY' && v.message.includes('너무 깁니다')
    );
