...
describe('AnalysisUtils Extra', () => {
  ...
  it('checkStructuralIntegrity: dg가 없는 경우 빈 배열을 반환해야 한다', async () => {
    expect(await checkStructuralIntegrity(undefined)).toEqual([]);
  });

  it('checkStructuralIntegrity: 순환 참조를 감지하여 위반 사항을 반환해야 한다', async () => {
    const mockDg = {
      detectCycles: vi.fn().mockReturnValue([['fileA.ts', 'fileB.ts', 'fileA.ts']]),
      getAllFiles: vi.fn().mockReturnValue([]),
      getDependencies: vi.fn().mockReturnValue([]),
    };
    const violations = await checkStructuralIntegrity(mockDg as any);
