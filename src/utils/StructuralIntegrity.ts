...
export async function checkStructuralIntegrity(dg?: DependencyGraph): Promise<Violation[]> {
  ...
  // Performance optimization: Used sequential for...of loops instead of Promise.all
  // to achieve true short-circuiting and avoid unnecessary parallel file reads when forwardRef is found early.
      ...
      if (existsSync(file)) {
        try {
          ...
          if (content.includes('forwardRef')) {
            hasForwardRef = true;
            break; // Short-circuit: no need to read remaining files in this cycle
          }
        } catch (e) {
          // 무시
        }
