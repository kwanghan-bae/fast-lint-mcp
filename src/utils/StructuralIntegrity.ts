...
export async function checkStructuralIntegrity(dg?: DependencyGraph): Promise<Violation[]> {
  ...
  // ⚡ Bolt: Used sequential for...of loops instead of Promise.all for early short-circuiting and to save I/O overhead.
      ...
      if (!hasForwardRef && existsSync(file)) {
        try {
          ...
          if (content.includes('forwardRef')) {
            hasForwardRef = true;
            break; // ⚡ Bolt: Early break when forwardRef is found
          }
        } catch (e) {
          // 무시
        }
