 ...
 * @returns 구조 위반 사항 목록
 */
export async function checkStructuralIntegrity(dg?: DependencyGraph): Promise<Violation[]> {
  const violations: Violation[] = [];
  if (!dg) return [];

  // 1. 모듈 간 순환 참조(Circular Dependency) 탐지 및 forwardRef 예외 처리
  const cycles = dg.detectCycles() || [];
  // ⚡ Bolt: Replace unbounded Promise.all with bounded pMap to prevent memory exhaustion and file descriptor limits
  await pMap(
    cycles,
    async (cycle) => {
      // 순환 경로 상에 존재하는 파일들의 내용을 읽어 forwardRef 사용 여부 확인
      // ⚡ Bolt: Use a sequential for...of loop to check for 'forwardRef' allowing for true early termination.
      // This prevents unnecessarily queueing 50 concurrent file reads via p-map when a match is found early.
