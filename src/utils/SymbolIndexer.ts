...
export class SymbolIndexer {
    ...
    async index() {
        ...
        const pattern = isAbsolute(this.workspacePath) ? join(this.workspacePath, 'src/**/*.{ts,js,tsx,jsx}') : 'src/**/*.{ts,js,tsx,jsx}';
        const files = await glob([pattern], { cwd: this.workspacePath, absolute: true, ignore: ['**/node_modules/**', '**/dist/**'] });
        
        for (const file of files) {
            try {
                const content = readFileSync(file, 'utf-8');
                const lang = (file.endsWith('.tsx') || file.endsWith('.jsx')) ? Lang.TypeScript : (file.endsWith('.ts') ? Lang.TypeScript : Lang.JavaScript);
    ...
    private traverse(node: SgNode, filePath: string) {
        ...
        if (kind === 'class_declaration' || kind === 'class') {
            if (!name || name === 'default') {
                name = node.find({ rule: { kind: "identifier" } })?.text().trim();
            }
