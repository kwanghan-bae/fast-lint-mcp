...
export async function analyzeFile(
  ...
  try {
    ...
    for (const rule of customRules) {
      // ⚡ Bolt: Replaced findAll().length > 0 with find() !== null
      // This short-circuits the AST traversal on the first match, avoiding full O(N) traversal and array allocation
