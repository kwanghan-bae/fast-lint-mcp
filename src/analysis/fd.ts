...
async function extractImportsFromFile(filePath: string, allFiles: string[]): Promise<string[]> {
  try {
    ...
    // ⚡ Bolt: Used AST node kind matching and m.field('source') instead of string patterns
    // to improve parsing performance and accuracy, eliminating O(N) regex evaluation.
