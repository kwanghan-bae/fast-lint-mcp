...
async function extractImportsFromFile(filePath: string, allFiles: string[]): Promise<string[]> {
  try {
    ...
    // ⚡ Bolt: Replaced string patterns with direct AST kind matching for better performance and correctness
    ...
    root.findAll({ rule: importRule }).forEach((m) => {
      ...
      if (sourceNode) {
        ...
        const source = sourceText.length >= 2 ? sourceText.slice(1, -1) : sourceText;
        if (source) {
