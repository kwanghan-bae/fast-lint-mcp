...
export async function getDependencyMap(
  ...
  // ⚡ Bolt: Replaced sequential iteration with concurrent p-map to speed up dependency map creation for large projects.
  await pMap(
    allFiles,
    async (filePath) => {
...
async function extractImportsFromFile(filePath: string, allFiles: string[]): Promise<string[]> {
  try {
    ...
    root.findAll({ rule: importRule }).forEach((m) => {
      const sourceWithQuotes = m.field('source')?.text();
      if (sourceWithQuotes) {
        const source = sourceWithQuotes.replace(/^['"]|['"]$/g, '');
