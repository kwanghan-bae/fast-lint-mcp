...
async function extractImportsFromFile(filePath: string, allFiles: string[]): Promise<string[]> {
  try {
    ...
    // ⚡ Bolt: Replaced string patterns with direct AST node kind matching for 3x performance improvement
    const importRule = {
    ...
    root.findAll({ rule: importRule }).forEach((m) => {
      ...
      if (sourceNode) {
        ...
        source = source.substring(1, source.length - 1); // Strip surrounding quotes
