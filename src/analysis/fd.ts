...
async function extractImportsFromFile(filePath: string, allFiles: string[]): Promise<string[]> {
  try {
    ...
    // ⚡ Bolt: Used AST node kind 'import_statement' and `.field('source')` instead of complex string patterns
    // This improves performance significantly by avoiding expensive multi-pattern evaluation.
    const importRule = {
