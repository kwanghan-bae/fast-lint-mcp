 ...
 */
export async function runMutationTest(filePath: string): Promise<{ id: string, message: string }[]> {
    ...
    if (originalContent.includes(original)) {
      ...
      try {
        ...
        try {
          ...
          violations.push({ id: 'MUTATION_SURVIVED', message: `변이 테스트 실패: '${original}'를 '${mutation}'로 바꿨는데도 테스트가 통과함. 테스트가 로직을 검증하지 못하고 있습니다.` });
        } catch (e) {
