  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: 'Analyzes complexity and line counts of functions/classes within a file. Use this to identify specific symbols for modification and save tokens.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description: 'Reads the source code content of a specific symbol (function, class). Prevents context waste by reading only necessary parts.',
        inputSchema: {
            type: 'object',
            properties: {
                filePath: { type: 'string', description: 'File path' },
                symbolName: { type: 'string', description: 'Target function or class name (e.g., myFunc, MyClass.method)' }
            },
      ...
      },
      {
        ...
        description: 'Identifies exported functions or variables that are not used anywhere in the project. Useful for technical debt cleanup.',
        inputSchema: {
            type: 'object',
            properties: {}
        }
      },
      {
        ...
        description: 'Tracks files and test cases affected by a symbol modification to prevent side effects during refactoring.',
        inputSchema: {
            type: 'object',
            properties: {
      ...
      },
      {
        ...
        description: 'Verifies if the modified code works correctly by executing a test command. Returns error logs on failure for self-healing.',
        inputSchema: {
          type: 'object',
          properties: {
            testCommand: { type: 'string', description: 'Test command to run (e.g., npm test, npx vitest run path/to/test)', default: 'npm test' }
          }
        }
      }
    ],
  ...
  try {
    ...
    switch (name) {
      ...
      case 'go-to-definition': {
        ...
        if (!def) {
          return { content: [{ type: 'text', text: `Definition for '${args?.symbolName}' not found.` }], isError: true };
        }
